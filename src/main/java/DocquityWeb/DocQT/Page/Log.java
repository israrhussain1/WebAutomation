...
import net.bytebuddy.utility.RandomString;
	...
	public class Log {
		...
		public static void afterMethodLogResult(Method method, ITestResult result, WebDriver driver)
				throws IOException {
			if (ITestResult.FAILURE == result.getStatus()) {
				String temp = takeScreenshot(method.getName(), driver);
				Log.extentLogger.log(Status.FAIL,
						MarkupHelper.createLabel(result.getName() + " FAILED ", ExtentColor.RED));
				Log.extentLogger.fail(result.getThrowable().getMessage(),
						MediaEntityBuilder.createScreenCaptureFromPath(temp).build());
			} else if (ITestResult.SUCCESS == result.getStatus()) {
				Log.extentLogger.log(Status.PASS,
						MarkupHelper.createLabel(result.getName() + " PASSED ", ExtentColor.GREEN));
			} else {
				Log.extentLogger.log(Status.SKIP,
